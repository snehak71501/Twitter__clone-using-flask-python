...
def account():
    ...
    #file_path = r'D:\final\Twitter-Clone\modules\verified.json'
    file_path = r'C:\Users\ELCOT\Downloads\Pendrive copy\final\Twitter-Clone\modules\verified.json'
    
    # Read data from the JSON file
    with open(file_path, 'r') as file:
        data = json.load(file)

    # Extract the list of verified usernames
    verified_usernames = data['verified_usernames']
    # Name to check
    name_to_check = current_user.username
    

    # Check if the name is in the list of verified usernames
    if name_to_check in verified_usernames:
        print(f"{name_to_check} is a verified username.")
    else:
        print(f"{name_to_check} is not a verified username.")
    ...
    return render_template('account.html',profile=profile_pic,background=bg_pic,update=update,timeline=all_posts, retweets=retweets,verified_usernames=verified_usernames)
